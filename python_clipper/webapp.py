...
try:
    from . import run as core
except ImportError:
    import run as core


app = Flask(__name__, static_folder="static", template_folder="templates")
READY_DIR = core.DEFAULT_READY_DIR
...
def run_job(job_id, payload):
    ...
    try:
        ...
        job_dir = os.path.join(READY_DIR, job_id)
        ...
        for idx, item in enumerate(targets, start=1):
            ...
            ok = core.proses_satu_clip(
                ...
                log_hook=lambda line: add_log(job_id, line),
            )
            if ok:
                success += 1
            else:
                add_log(job_id, f"Clip {idx} gagal dibuat.")
            set_job(job_id, done=idx, success=success, outputs=list_outputs(job_dir))

        outputs = list_outputs(job_dir)
        if success == 0 or not outputs:
            recent_logs = "\n".join(jobs.get(job_id, {}).get("logs", [])[-8:])
            raise RuntimeError(
                "Processing finished, but no clip file was generated. "
                "Check the selected segment, FFmpeg/yt-dlp availability, or subtitle settings."
        ...
        manifest = manifest_helper.write_job_manifest(
            ...
            approval_mode=payload.get("approval_mode") or "required",
        ...
        set_job(
            ...
            error=str(e),
            finished_at=now_ms(),
            outputs=list_outputs(job_dir) if job_dir else [],
        )
